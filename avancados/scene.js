...
echoScene.on('message', ctx => {
    ctx.reply(`Desculpa, eu so aceito mensagem de texto, agradeço`)
})

let sum = 0
const sumScene = new Scene('sum')
sumScene.enter(ctx => ctx.reply('Entrando em Sum Scene'))
sumScene.leave(ctx => ctx.reply('Saindo da Sum Scene'))

sumScene.use(async (ctx, next) => {
    await ctx.reply('Você esta em Soma Scene, escreva números para somar')
